...
public class EnemyUnit : MonoBehaviour
{
    ...
    public void Setup()
    {
        //If isDog then use new set of skill
        Enemy = new Enemy(enemyBase[Random.Range(0,enemyBase.Length)], (BattleHUD.isDog ? 1 : 0));

        if (!isPlayerUnit)
        {
            image.sprite = Enemy.EnemyBase.FrontSprite;
            image.SetNativeSize();
        }
